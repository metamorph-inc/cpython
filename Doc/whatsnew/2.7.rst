...
:option:`-Wd <-W>`) switch, or by setting the :envvar:`OMPYTHONWARNINGS`
...
A new environment variable, :envvar:`OMPYTHONWARNINGS`,
...
::
  ...
  export OMPYTHONWARNINGS=all,error:::Cookie:0
...
:envvar:`OMPYTHONSHOWREFCOUNT` environment variable now must also be set.
(Contributed by Victor Stinner; :issue:`31733`.)

When Python is compiled with ``COUNT_ALLOC`` defined, allocation counts are no
longer dumped by default anymore: the :envvar:`OMPYTHONSHOWALLOCCOUNT` environment
...
``OMPYTHONHTTPSVERIFY`` environment variable may be set to ``0`` to revert an
